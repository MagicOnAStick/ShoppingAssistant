		...
		Profile.findOne({ user: req.user.id }).then(profile => {
			if (profile) {
				...
				Profile.findOne({ handle: profileFields.handle }).then(profile => {
					if (profile) {
						errors.handle =
							"Handle already exists - please choose another handle!";
...
//@access private (user required)
router.post(
	"/weekplan/:weekplan_id",
	passport.authenticate("jwt", { session: false }),
	(req, res) => {
		Profile.findOne({ user: req.user.id }).then(profile => {
			Weekplan.findById(req.params.weekplan_id).then(plan => {
...
router.post(
	...
	(req, res) => {
		Profile.findOne({ user: req.user.id }).then(profile => {
			Achievement.findById(req.params.achievement_id).then(achievement => {
